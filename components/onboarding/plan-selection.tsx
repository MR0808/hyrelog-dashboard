...
import { createCheckoutSession, assignFreePlan } from '@/app/actions/stripe';
...
export function PlanSelection({ plans }: PlanSelectionProps) {
    ...
    const handleContinue = () => {
        ...
        const selectedPlanData = selfServePlans.find(
            (p) => p.id === selectedPlan
        );
        if (selectedPlanData?.priceCents === 0) {
            // Assign free plan
            startTransition(async () => {
                const result = await assignFreePlan(selectedPlan);

                if (result.success) {
                    router.push('/onboarding/workspace');
                } else {
                    setError(result.error || 'Failed to assign free plan');
                }
            });
            return;
        }
