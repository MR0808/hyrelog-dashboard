...
export default async function BillingPage() {
  ...
  const usageData = data.usageStats
    .reverse()
    .map((stat) => ({
      period: new Date(stat.periodStart).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Usage & Billing</h1>
        <p className="text-muted-foreground">Monitor your usage and billing information</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {data.plans?.billingCycle === 'ANNUAL' ? 'per year' : 'per month'}
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{data.addOns.length}</div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Events ingested and queried by period</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              {data.addOns.map((addOn) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{addOn.addOn.description}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-medium">${((addOn.addOn.priceCents * addOn.quantity) / 100).toFixed(2)}</p>
                    <p className="text-sm text-muted-foreground">Qty: {addOn.quantity}</p>
                  </div>
                </div>
              ))}
            </div>
