...
export const AlertThresholdEmail = ({
  ...
  const percentage = threshold > 0 ? Math.round((currentUsage / threshold) * 100) : 0;
  
  return (
    <Html>
      <Head />
      <Preview>
        {companyName} has reached {thresholdType === 'soft' ? 'soft' : 'hard'} limit for {meterType}
      </Preview>
      <Body style={main}>
        <Container style={container}>
          ...
          <Text style={text}>
            ...
            <strong>{thresholdType === 'soft' ? 'soft' : 'hard'}</strong> limit
            for <strong>{meterType}</strong>.
          </Text>
          <Section style={statsContainer}>
            <Text style={statLabel}>Current Usage:</Text>
            <Text style={statValue}>{currentUsage.toLocaleString()}</Text>
            <Text style={statLabel}>Threshold:</Text>
            <Text style={statValue}>{threshold.toLocaleString()}</Text>
            <Text style={statLabel}>Percentage:</Text>
            <Text style={statValue}>{percentage}%</Text>
          </Section>
          {thresholdType === 'hard' && (
            <Text style={warningText}>
              ⚠️ You've reached the hard limit. Some features may be restricted
              until usage is reduced or limits are increased.
            </Text>
          )}
